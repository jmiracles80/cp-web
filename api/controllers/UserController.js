...
module.exports = {
  ...
  if (req.param('encryptedPassword').length < 6) {
  ...
  var hashpswd = bcrypt.hashSync(req.param('encryptedPassword'), 10);
...
login: function(req, res) {
    User.findOne({
        ...
        if (!user) return res.notFound();
					...
					try {
						bcrypt.compare(req.param('encryptedPassword'), user.encryptedPassword, function(err, res) {
							// res == true
							if (err) {
						...
						/*********************
						 FIX ME
						 ******************/
						 //******Figure out why negotiate is not working????
						//	return res.negotiate(err);
						// TypeError: Cannot call method 'negotiate' of undefined
							// at Object.<anonymous> (/home/jmiracles80/cp-web/api/controllers/UserController.js:192:23)
